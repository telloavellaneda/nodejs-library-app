...
public class FormElements {
	...
	public String createSelectElement() {
		...
		String options = "";
		if ( tmp != null && !control.isOnlyValue() ) {
			for (Iterator<Catalogo> iterator = tmp; iterator.hasNext();) {
				Catalogo catalogo = iterator.next();
				String tempLabel = getOptionLabel(catalogo.getNombre(), control.getLabelLength());
				String selected = ( catalogo.getId().equals( control.getValue() ) ) ? " selected" : "" ;
				options += TAB + TAB + TAB + TAB + "<option value=\"" + catalogo.getId() + "\"" + selected + ">" + tempLabel + "</option>" + CR;				
			}

		} else if ( tmp != null && control.isOnlyValue() ) {
			for (Iterator<Catalogo> iterator = tmp; iterator.hasNext();) {
				Catalogo catalogo = iterator.next();				
	...
	private String getOptionLabel(String label, int length) {
		if ( label != null && !label.equals("") && length > 0 && label.length() > length )
